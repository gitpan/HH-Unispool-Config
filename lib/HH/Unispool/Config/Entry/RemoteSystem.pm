package HH::Unispool::Config::Entry::RemoteSystem;
...
# Used by _initialize
our %DEFAULT_VALUE = (
    'execution_priority' => HH::Unispool::Config::ExecPri->new( {
        execution_priority => '',
    } ),
);
...
HH::Unispool::Config::Entry::RemoteSystem - Abstract class to group UNISPOOL remote system entries
...
Abstract class for UNISPOOL remote system entries

=head1 DESCRIPTION

C<HH::Unispool::Config::Entry::RemoteSystem> is an abstract class to group UNISPOOL remote system entries.
...
Creates a new C<HH::Unispool::Config::Entry::RemoteSystem> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
Returns the description for the remote system.
...
Set the description for the remote system. C<VALUE> is the value. On error an exception C<Error::Simple> is thrown.
...
sub new_from_tokenizer {
    throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::new_from_tokenizer, call this method in a subclass that has implemented it.");
}
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::_initialize, first argument must be 'HASH' reference.");
...
sub diff {
    ...
    if ( ref($from) ne ref($to) ) {
        ...
        throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::diff, FROM ($rf) and TO ($rt) reference types differ.");
    }
...
sub get_description {
    ...
    return( $self->{HH_Unispool_Config_Entry_RemoteSystem}{description} );
}

sub get_execution_priority {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_RemoteSystem}{execution_priority} );
}
...
sub set_description {
    ...
    &_value_is_allowed( 'description', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::set_description, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_RemoteSystem}{description} = $val;
}
...
sub set_execution_priority {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'execution_priority', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::set_execution_priority, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_RemoteSystem}{execution_priority} = $val;
}

sub write {
    throw Error::Simple("ERROR: HH::Unispool::Config::Entry::RemoteSystem::write, call this method in a subclass that has implemented it.");
}
