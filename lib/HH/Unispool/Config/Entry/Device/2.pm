package HH::Unispool::Config::Entry::Device::2;
...
Class C<HH::Unispool::Config::Entry::Device::2> holds information for UNISPOOL type 2 device
...
Creates a new C<HH::Unispool::Config::Entry::Device::2> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::2::_initialize, first argument must be 'HASH' reference.");
    ...
    exists( $opt->{device_file} ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::2::_initialize, option 'device_file' is mandatory.");
...
sub diff {
    ...
    if ( ref($from) ne ref($to) ) {
        ...
        throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::2::diff, FROM ($rf) and TO ($rt) reference types differ.");
    }
...
sub write {
    ...
    my $d = HH::Unispool::Config::File::Token::Numbered::Device::2->new( {
