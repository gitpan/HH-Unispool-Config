package HH::Unispool::Config::Entry::Device::4;
...
HH::Unispool::Config::Entry::Device::4 - UNISPOOL type 4 device
...
UNISPOOL type 4 device

=head1 DESCRIPTION

Class C<HH::Unispool::Config::Entry::Device::4> holds information for UNISPOOL type 4 device
...
Creates a new C<HH::Unispool::Config::Entry::Device::4> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::_initialize, first argument must be 'HASH' reference.");

    # destination_device, SINGLE, mandatory
    exists( $opt->{destination_device} ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::_initialize, option 'destination_device' is mandatory.");
...
sub diff {
    ...
    if ( ref($from) ne ref($to) ) {
        ...
        throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::diff, FROM ($rf) and TO ($rt) reference types differ.");
    }
...
sub new_from_tokenizer {
    ...
    # First token must be a HH::Unispool::Config::File::Token::Numbered::Device::4
    my $s = $tokenizer->get();
    $s->isa('HH::Unispool::Config::File::Token::Numbered::Device::4') || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::new_from_tokenizer, expected a first token from 'TOKENIZER' of class 'HH::Unispool::Config::File::Token::Numbered::Device::4'.");
    ...
    while ( my $tok = $tokenizer->get() ) {
        if ( ! $tok->isa('HH::Unispool::Config::File::Token::Numbered') || $tok->get_number() != $s->get_number() ) {
            ...
            defined ($x) && throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::new_from_tokenizer, multiple tokens obtained from 'TOKENIZER' for entry $opt{name}/$opt{number} from class 'HH::Unispool::Config::File::Token::Numbered::X'.");
            $x = $tok;
            ...
            defined ($i) && throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::new_from_tokenizer, multiple tokens obtained from 'TOKENIZER' for entry $opt{name}/$opt{number} from class 'HH::Unispool::Config::File::Token::Numbered::Device::Info'.");
            $i = $tok;
            $opt{description} = $i->get_description();
        }
        else {
            throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::new_from_tokenizer, expected tokens from 'TOKENIZER' for entry $opt{name}/$opt{number} from either class 'HH::Unispool::Config::File::Token::Numbered::Device::Info' or 'HH::Unispool::Config::File::Token::Numbered::Network'.");
        }
    }

    # Construct a new object and return it
    return( HH::Unispool::Config::Entry::Device::4->new(\%opt) );
}
...
sub write {
    ...
    my $d = HH::Unispool::Config::File::Token::Numbered::Device::4->new( {
...
sub set_destination_device {
    ...
    &_value_is_allowed( 'destination_device', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::set_destination_device, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_4}{destination_device} = $val;
}

sub get_destination_device {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_4}{destination_device} );
}
...
sub set_execution_priority {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'execution_priority', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::set_execution_priority, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_4}{execution_priority} = $val;
}

sub get_execution_priority {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_4}{execution_priority} );
}
...
sub set_header_name {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'header_name', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::set_header_name, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_4}{header_name} = $val;
}

sub get_header_name {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_4}{header_name} );
}
...
sub set_initially_spooled {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_4}{initially_spooled} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_4}{initially_spooled} = 0;
    }
}
...
sub is_initially_spooled {
    ...
    if ( $self->{HH_Unispool_Config_Entry_Device_4}{initially_spooled} ) {
        return(1);
    }
    else {
        return(0);
...
sub set_networkwide {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_4}{networkwide} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_4}{networkwide} = 0;
    }
}
...
sub is_networkwide {
    ...
    if ( $self->{HH_Unispool_Config_Entry_Device_4}{networkwide} ) {
        return(1);
    }
    else {
        return(0);
...
sub set_save_printfile {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_4}{save_printfile} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_4}{save_printfile} = 0;
    }
}
...
sub is_save_printfile {
    ...
    if ( $self->{HH_Unispool_Config_Entry_Device_4}{save_printfile} ) {
        return(1);
    }
    else {
        return(0);
...
sub set_trailer_name {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'trailer_name', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::4::set_trailer_name, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_4}{trailer_name} = $val;
}

sub get_trailer_name {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_4}{trailer_name} );
}
