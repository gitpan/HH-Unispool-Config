package HH::Unispool::Config::Entry::Device::0;
...
Class C<HH::Unispool::Config::Entry::Device::0> holds information for UNISPOOL type 0 device
...
Creates a new C<HH::Unispool::Config::Entry::Device::0> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.

Options for C<OPT_HASH_REF> may include:

=over

=item B<C<device_file>>

Passed to L<set_device_file()>. Mandatory option.
...
Passed to L<set_unispool_control()>. Defaults to B<0>.
...
Set the device file to which the device is connected. C<VALUE> is the value. On error an exception C<Error::Simple> is thrown.
...
State that the UNISPOOL control is on (is not available on development system used). C<VALUE> is the value. Default value at initialization is C<0>. On error an exception C<Error::Simple> is thrown.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::_initialize, first argument must be 'HASH' reference.");

    # device_file, SINGLE, mandatory
    exists( $opt->{device_file} ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::_initialize, option 'device_file' is mandatory.");
    $self->set_device_file( $opt->{device_file} );
    ...
    # unispool_control, BOOLEAN, with default value
    $self->set_unispool_control( exists( $opt->{unispool_control} ) ? $opt->{unispool_control} : $DEFAULT_VALUE{unispool_control} );
...
sub diff {
    ...
    if ( ref($from) ne ref($to) ) {
        ...
        throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::diff, FROM ($rf) and TO ($rt) reference types differ.");
    }

    # Diff message
    my $diff = $from->SUPER::diff($to, $diff_number);

    # Diff the device file
    if ( $from->get_device_file() ne $to->get_device_file() ) {
        my $ref = ref($from);
        my $vf = $from->get_device_file();
        my $vt = $to->get_device_file();
        my $name = $from->get_name();
        my $number = $from->get_number();
        $diff .= "$ref/$name/$number: device file difference: $vf <-> $vt\n";
    }
    ...
    # Diff the unispool control state
    if ( $from->is_unispool_control() != $to->is_unispool_control() ) {
        my $ref = ref($from);
        my $vf = $from->is_unispool_control();
        my $vt = $to->is_unispool_control();
        my $name = $from->get_name();
        my $number = $from->get_number();
        $diff .= "$ref/$name/$number: unispool control difference: $vf <-> $vt\n";
    }
...
sub set_device_file {
    ...
    &_value_is_allowed( 'device_file', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::set_device_file, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_0}{device_file} = $val;
}

sub get_device_file {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_0}{device_file} );
}
...
sub set_device_password {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'device_password', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::set_device_password, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_0}{device_password} = $val;
}

sub get_device_password {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_0}{device_password} );
}
...
sub set_execution_priority {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'execution_priority', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::set_execution_priority, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_0}{execution_priority} = $val;
}

sub get_execution_priority {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_0}{execution_priority} );
}
...
sub set_header_name {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'header_name', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::set_header_name, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_0}{header_name} = $val;
}

sub get_header_name {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_0}{header_name} );
}
...
sub set_initially_spooled {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_0}{initially_spooled} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_0}{initially_spooled} = 0;
    }
}
...
sub set_networkwide {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_0}{networkwide} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_0}{networkwide} = 0;
    }
}
...
sub set_save_printfile {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_0}{save_printfile} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_0}{save_printfile} = 0;
    }
}
...
sub set_trailer_name {
    ...
    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'trailer_name', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::Entry::Device::0::set_trailer_name, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_Entry_Device_0}{trailer_name} = $val;
}

sub get_trailer_name {
    my $self = shift;

    return( $self->{HH_Unispool_Config_Entry_Device_0}{trailer_name} );
}
...
sub set_unispool_control {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_0}{unispool_control} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_0}{unispool_control} = 0;
    }
}
...
sub set_unispool_header {
    ...
    if (shift) {
        $self->{HH_Unispool_Config_Entry_Device_0}{unispool_header} = 1;
    }
    else {
        $self->{HH_Unispool_Config_Entry_Device_0}{unispool_header} = 0;
    }
}
