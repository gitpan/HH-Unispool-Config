package HH::Unispool::Config::File::Token::Comment::Head;
...
HH::Unispool::Config::File::Token::Comment::Head - file head comment token
...
file head comment token

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Comment::Head> contains information on the file head comment token.
...
Creates a new C<HH::Unispool::Config::File::Token::Comment::Head> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
Passed to L<set_version()>. Mandatory option.
...
Set the host in the comment. C<VALUE> is the value. C<VALUE> may not be C<undef>. On error an exception C<Error::Simple> is thrown.
...
Set the version in the comment. C<VALUE> is the value. C<VALUE> may not be C<undef>. On error an exception C<Error::Simple> is thrown.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Head::_initialize, first argument must be 'HASH' reference.");

    # host, SINGLE, mandatory
    exists( $opt->{host} ) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Head::_initialize, option 'host' is mandatory.");
    $self->set_host( $opt->{host} );

    # version, SINGLE, mandatory
    exists( $opt->{version} ) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Head::_initialize, option 'version' is mandatory.");
    $self->set_version( $opt->{version} );
