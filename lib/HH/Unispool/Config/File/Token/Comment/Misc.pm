package HH::Unispool::Config::File::Token::Comment::Misc;
...
HH::Unispool::Config::File::Token::Comment::Misc - miscelaneous section comment token
...
miscelaneous section comment token

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Comment::Misc> contains information on the miscelaneous section comment token.
...
Creates a new C<HH::Unispool::Config::File::Token::Comment::Misc> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
=head1 INHERITED METHODS FROM HH::Unispool::Config::File::Token::Comment::NetDesc

=over

=item To access attribute named B<C<net_desc>>:

set_net_desc(), get_net_desc()
...
sub read_string {
    ...
    # Parse line
    my ($net_desc) = $line =~ /$USP_CFG_MISC_RX/;
    defined($net_desc) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Misc::read_string, parameter 'LINE' does not match the regular expression for this token's line string.");
