package HH::Unispool::Config::File::Token::Comment::Tail;
...
HH::Unispool::Config::File::Token::Comment::Tail - file tail comment token
...
file tail comment token

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Comment::Tail> contains information on the file tail comment token.
...
Creates a new C<HH::Unispool::Config::File::Token::Comment::Tail> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
Passed to L<set_host()>. Mandatory option.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Tail::_initialize, first argument must be 'HASH' reference.");

    # host, SINGLE, mandatory
    exists( $opt->{host} ) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Tail::_initialize, option 'host' is mandatory.");
...
sub get_host {
    ...
    return( $self->{HH_Unispool_Config_File_Token_Comment_Tail}{host} );
}
...
sub read_string {
    ...
    defined($host) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Tail::read_string, parameter 'LINE' does not match the regular expression for this token's line string.");
...
sub set_host {
    ...
    defined($val) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Tail::set_host, value may not be empty.");

    # Check if isa/ref/rx/value is allowed
    &_value_is_allowed( 'host', $val ) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Comment::Tail::set_host, the specified value '$val' is not allowed.");

    # Assignment
    $self->{HH_Unispool_Config_File_Token_Comment_Tail}{host} = $val;
}
