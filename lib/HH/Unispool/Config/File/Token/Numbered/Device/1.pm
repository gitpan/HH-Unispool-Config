package HH::Unispool::Config::File::Token::Numbered::Device::1;
...
HH::Unispool::Config::File::Token::Numbered::Device::1 - device type C<1> device I<D> token
...
device type C<1> device I<D> token

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Numbered::Device::1> contans device type C<1> device I<D> token information.
...
Creates a new C<HH::Unispool::Config::File::Token::Numbered::Device::1> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
sub read_string {
    ...
    defined($number) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::1::read_string, parameter 'LINE' does not match the regular expression for this token's line string.");
    $device_name =~ s/\s*$//;
    ...
    defined($flags) && ( $flags =~ /i/i ) && throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::1::read_string, flag 'I' is not allowed in the token's line string.");
    defined($flags) && ( $flags =~ /u/i ) && throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::1::read_string, flag 'U' is not allowed in the token's line string.");
