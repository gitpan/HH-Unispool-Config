package HH::Unispool::Config::File::Token::Numbered::Device::4;
...
HH::Unispool::Config::File::Token::Numbered::Device::4 - device type C<4> device I<D> token
...
device type C<4> device I<D> token

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Numbered::Device::4> contans device type C<4> device I<D> token information.
...
Creates a new C<HH::Unispool::Config::File::Token::Numbered::Device::4> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. C<OPT_HASH_REF> is mandatory. On error an exception C<Error::Simple> is thrown.
...
sub read_string {
    ...
    # Parse line for name
    my ($number, $device_name, $tail) = $line =~ /$USP_D4_RX/;
    defined($number) || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::4::read_string, parameter 'LINE' does not match the regular expression for this token's line string.");
    $device_name =~ s/\s*$//;
    my @tail = $self->_split_tail($tail);
    my $remote_device_name = $tail[6];
    my $flags = $tail[7];
    my $execution_priority = $tail[8];
    defined($flags) && ( $flags =~ /h/i ) && throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::4::read_string, flag 'H' is not allowed in the token's line string.");
    defined($flags) && ( $flags =~ /u/i ) && throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Numbered::Device::4::read_string, flag 'U' is not allowed in the token's line string.");
