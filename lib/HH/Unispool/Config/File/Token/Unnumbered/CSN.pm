package HH::Unispool::Config::File::Token::Unnumbered::CSN;
...
HH::Unispool::Config::File::Token::Unnumbered::CSN - class for console message system I<L> tokens
...
class for console message system I<L> tokens

=head1 DESCRIPTION

C<HH::Unispool::Config::File::Token::Unnumbered::CSN> is a class for console message system I<L> tokens.
...
Creates a new C<HH::Unispool::Config::File::Token::Unnumbered::CSN> object. C<OPT_HASH_REF> is a hash reference used to pass initialization options. On error an exception C<Error::Simple> is thrown.
...
Passed to L<set_central_console_node()>.
...
sub _initialize {
    ...
    # Check $opt
    ref($opt) eq 'HASH' || throw Error::Simple("ERROR: HH::Unispool::Config::File::Token::Unnumbered::CSN::_initialize, first argument must be 'HASH' reference.");

    # central_console_node, SINGLE
    exists( $opt->{central_console_node} ) && $self->set_central_console_node( $opt->{central_console_node} );
