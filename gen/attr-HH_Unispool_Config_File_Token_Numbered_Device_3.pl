...
my $pkg = 'HH::Unispool::Config::File::Token::Numbered::Device::3';

push (@::bean_desc, {
    bean_opt => {
        package => $pkg,
        base => [qw (HH::Unispool::Config::File::Token::Numbered::Device)],
        short_description => 'device type C<3> device I<D> token',
        abstract => 'device type C<3> device I<D> token',
        synopsis => "Application programmers don't need to use this class and API programmers read code.\n",
        description => <<EOF,
C<${pkg}> contans device type C<3> device I<D> token information.
EOF
    },
    attr_opt => [
    ],
    ...
    defined(\$flags) && ( \$flags =~ /s/i ) && throw Error::Simple("ERROR: ${pkg}::read_string, flag 'S' is not allowed in the token's line string.");
