...
my $pkg = 'HH::Unispool::Config::File::Token::Numbered::System::6';

push (@::bean_desc, {
    bean_opt => {
        package => $pkg,
        base => [qw (HH::Unispool::Config::File::Token::Numbered::System)],
        short_description => 'system type C<6> system I<S> token',
        abstract => 'system type C<6> system I<S> token',
        synopsis => "Application programmers don't need to use this class and API programmers read code.\n",
        description => <<EOF,
C<${pkg}> contans system type C<6> system I<S> token information.
EOF
    },
    attr_opt => [
    ],
