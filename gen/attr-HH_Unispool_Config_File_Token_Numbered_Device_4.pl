...
my $pkg = 'HH::Unispool::Config::File::Token::Numbered::Device::4';

push (@::bean_desc, {
    bean_opt => {
        package => $pkg,
        base => [qw (HH::Unispool::Config::File::Token::Numbered::Device)],
        short_description => 'device type C<4> device I<D> token',
        abstract => 'device type C<4> device I<D> token',
        synopsis => "Application programmers don't need to use this class and API programmers read code.\n",
        description => <<EOF,
C<${pkg}> contans device type C<4> device I<D> token information.
EOF
    },
    attr_opt => [
    ],
