...
my $pkg = 'HH::Unispool::Config::File::Token::Comment::Cs';

push (@::bean_desc, {
    bean_opt => {
        package => $pkg,
        base => [qw (HH::Unispool::Config::File::Token::Comment::NetDesc)],
        short_description => 'configuration server comment token',
        abstract => 'configuration server comment token',
        synopsis => "Application programmers don't need to use this class and API programmers read code.\n",
        description => <<EOF,
C<${pkg}> contains information on the configuration server comment token.
EOF
    },
    attr_opt => [
    ],
