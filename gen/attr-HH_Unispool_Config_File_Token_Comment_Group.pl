...
my $pkg = 'HH::Unispool::Config::File::Token::Comment::Group';

push (@::bean_desc, {
    bean_opt => {
        package => $pkg,
        base => [qw (HH::Unispool::Config::File::Token::Comment::NetDesc)],
        short_description => 'group section comment token',
        abstract => 'group section comment token',
        synopsis => "Application programmers don't need to use this class and API programmers read code.\n",
        description => <<EOF,
C<${pkg}> contains information on the group section comment token.
EOF
    },
    attr_opt => [
    ],
